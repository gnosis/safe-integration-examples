...
import { loadTokenInfo, loadTokenAllowance, TokenInfo, setAllowance, buildAllowanceTx } from '../logic/tokens'
import { uniswapContract } from '../logic/config'
import { buildSwapTokensTx, swapTokens } from '../logic/uniswap'
import { useSafeAppsSDK } from '@gnosis.pm/safe-apps-react-sdk';
import { SafeAppsSdkProvider } from '@gnosis.pm/safe-apps-ethers-provider';
import { waitForSafeTx } from '../logic/safeTx'
...
const Root = () => {
  ...
  const provider = useMemo(() => {
    return connected ? new SafeAppsSdkProvider(safe, sdk) : new ethers.providers.Web3Provider(window.ethereum)
  }, [safe, sdk, connected, window.ethereum])
  ...
  const approveAndTrade = useCallback(async () => {
    if (!validatedInputs.buyToken || !validatedInputs.sellToken || !validatedInputs.tokenAmount) return
    try {
      const description = `Swapping ${formatUnits(validatedInputs.tokenAmount, validatedInputs.sellToken.decimals)} ${validatedInputs.sellToken.symbol} for ${validatedInputs.buyToken.symbol}`
      setPendingTrade({
      ...
      const allowanceTx = buildAllowanceTx(validatedInputs.sellToken.address, await uniswapContract(provider), validatedInputs.tokenAmount)
      const swapTokenTx = await buildSwapTokensTx(
        signer,
        provider,
        validatedInputs.sellToken.address,
        validatedInputs.buyToken.address,
      ...
      await waitForSafeTx(sdk, safeTxHash)
    } catch (e) {
      console.error(e)
    } finally {
      setPendingTrade(undefined)
    }
  }, [sdk, provider, validatedInputs, setPendingTrade])
    ...
    <>
      ...
      <Card className={classes.card}>
        ...
        <CardActions>
          ...
          { connected && (
            <Button size="small" color="primary" disabled={!!pendingTrade || !validatedInputs.buyToken || !validatedInputs.sellToken || !validatedInputs.tokenAmount} onClick={approveAndTrade}>
              Approve+Trade
            </Button>
          )}
        </CardActions>
