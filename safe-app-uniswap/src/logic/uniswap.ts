...
export const buildSwapTokensTx = async(signer: Signer, provider: providers.Provider, sellToken: string, buyToken: string, amount: BigNumber) => {
    const uniswapAddress = await uniswapContract(provider)
    const reveiver = await signer.getAddress()
    return {
        to: uniswapAddress,
        value: "0",
        data: UniswapInterface.encodeFunctionData("swapExactTokensForTokens", [amount, 0, [sellToken, buyToken], reveiver, BigNumber.from("0xFFFFFFFFFFFFFFFF")])
    }
}